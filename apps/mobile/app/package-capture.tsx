 ...
 * Her adımda neden gerektiği ve atlanırsa ne olacağı yazılır; tekrar çekme ve atlama vardır.
 * OCR bu sürümde YOK: aday metin elle yazılır veya (yalnız __DEV__ + fixture bayrağı) fixture
 * ile ön doldurulur ve "GELİŞTİRME FIXTURE" etiketiyle görünür. Sonuç `candidate` taslaktır;
 * dış servise yüklenmez, skorlara ve alerjen kararına girmez.
 */
...
export default function PackageCaptureScreen() {
  ...
  const initialBarcode = typeof params.barcode === 'string' && /^\d{8,14}$/.test(params.barcode) ? params.barcode : null;
  ...
  useEffect(() => {
    if (phase === 'review' && fixtureEnabled) {
      const next: Partial<Record<DraftTextField, string>> = {};
      for (const c of ocrCandidateFixture) {
        if (c.text) next[c.field] = c.text;
      }
      setTexts((current) => ({ ...next, ...current }));
    }
  ...
  const handleBarcodeScanned = (result: BarcodeScanningResult) => {
    ...
    if (/^\d{8,14}$/.test(result.data)) setGtin(result.data);
  };
  ...
  const handleSaveDraft = async () => {
    ...
    await saveContributionDraft(created);
    setDraft(created);
    setPhase('done');
  };
  ...
  if (!permission) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.body}>Kamera izni kontrol ediliyor...</Text>
      </View>
    );
  }
  ...
  if (!permission.granted && phase === 'capture') {
    return (
      <View style={styles.centered}>
        <Text style={styles.title}>Paket bilgisini ekle</Text>
        <Text style={styles.body}>Ambalaj fotoğrafı için kamera izni gerekli. Fotoğraflar cihazda kalır; bu sürümde yüklenmez.</Text>
        <Pressable style={styles.primaryButton} accessibilityRole="button" accessibilityLabel="Kamera izni ver" onPress={requestPermission}>
          <Text style={styles.primaryButtonText}>Kamera izni ver</Text>
        </Pressable>
        <Pressable style={styles.secondaryButton} accessibilityRole="button" accessibilityLabel="Fotoğrafsız devam et, metni elle yaz" onPress={() => setPhase('review')}>
  ...
  if (phase === 'review') {
    return (
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        />
        {!gtin ? <Text style={styles.warnText}>Barkod olmadan taslak kaydedilemez.</Text> : null}
        ...
        <Pressable
          style={[styles.primaryButton, !gtin ? styles.buttonDisabled : null]}
          accessibilityRole="button"
          accessibilityLabel="Taslağı kaydet"
          accessibilityState={{ disabled: !gtin }}
          disabled={!gtin}
          onPress={() => void handleSaveDraft()}
        >
          <Text style={styles.primaryButtonText}>Taslağı kaydet (aday, gönderilmez)</Text>
        </Pressable>
        <Pressable style={styles.secondaryButton} accessibilityRole="button" accessibilityLabel="Fotoğraf adımlarına dön" onPress={() => setPhase('capture')}>
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.cameraWrapper}>
        {currentPhoto ? (
          <View style={styles.photoTaken} accessible accessibilityLabel={`${step.title} fotoğrafı çekildi`}>
            <Text style={styles.photoTakenIcon}>✔</Text>
            <Text style={styles.photoTakenText}>Fotoğraf çekildi</Text>
          </View>
        ) : (
          <CameraView
            ref={cameraRef}
      ...
      <View style={styles.actions}>
        ...
        {currentPhoto || (step.kind === 'barcode' && gtin) ? (
          <Pressable style={styles.primaryButton} accessibilityRole="button" accessibilityLabel="Sonraki adım" onPress={goNext}>
...
const styles = StyleSheet.create({
  ...
  photoTakenIcon: { fontSize: 36, color: '#FFFFFF', fontWeight: '700' },
