...
- **Mobile lactose classification tracks milk's status (2026-09-22, device-test finding).** `apps/mobile/src/riskEngine/catalogAllergenChip.ts`'s `classifyForProfileKey` mirrors the profile's `lactose` key to whatever status `milk` has, per the Türk Gıda Kodeksi (TGK) Etiketleme Yönetmeliği's "süt ve süt ürünleri (laktoz dahil)" wording: milk declared → lactose `declared`; milk trace → lactose `trace`; product `present` and milk not in declared/traces → lactose `not_listed_in_available_data`; data `partial`/`unknown_or_unverified` → lactose `unknown_or_unverified` too. The `declared`/`trace` badge and note text deliberately never say "laktoz içerir" — they name milk instead (`displayLabelForKey`), since the evidence is about milk's declaration, not a direct lactose measurement. Ingredients text containing "laktoz" independently triggers a warning via `ingredientsMatchKey('lactose', ...)`, which calls into `riskEngine.ts`'s `LACTOSE_KEYWORDS` naive substring matcher — this is a **known false positive** for "laktozsuz" (lactose-free), since "laktozsuz" contains "laktoz" as a substring; accepted deliberately because the error direction is conservative (over-warning, not under-warning), per D1. **This TGK-derived mapping has not been reviewed by a food-safety/regulatory expert against the actual TGK text** (same caveat as `TR_ALLERGEN_SYNONYMS` above) — flagged here for that same future review. See `lactoseTracksMilk.smoke.ts` for the exact scenarios and the accepted false positive.
